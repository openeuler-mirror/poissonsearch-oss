...
----
# Stop Elasticsearch
sudo service poissonsearch stop

# Install the plugin
sudo /usr/share/poissonsearch/bin/elasticsearch-plugin install discovery-azure-classic

# Configure it
sudo vi /etc/poissonsearch/elasticsearch.yml
----
...
cloud:
    azure:
        management:
             ...
             keystore:
                   path: /home/poissonsearch/azurekeystore.pkcs12
...
# path.data: /mnt/resource/poissonsearch/data
----
...
[source,sh]
----
sudo service poissonsearch start
----

If anything goes wrong, check your logs in `/var/log/poissonsearch`.
