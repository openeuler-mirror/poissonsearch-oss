...
`/etc/poissonsearch`. The location of the config directory can also be changed
...
`/etc/default/poissonsearch` (for the Debian package) and
`/etc/sysconfig/poissonsearch` (for the RPM package). You will need to edit the
`ES_PATH_CONF=/etc/poissonsearch` entry in one of these files accordingly to
...
[source,yaml]
--------------------------------------------------
path.data: /var/lib/poissonsearch
path.logs: /var/log/poissonsearch
--------------------------------------------------
