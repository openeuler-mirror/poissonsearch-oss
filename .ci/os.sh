...
sudo bash -c 'cat > /etc/sudoers.d/poissonsearch_vars'  << SUDOERS_VARS
    Defaults   env_keep += "JAVA_HOME"
    Defaults   env_keep += "SYSTEM_JAVA_HOME"
SUDOERS_VARS
sudo chmod 0440 /etc/sudoers.d/poissonsearch_vars

# Bats tests still use this locationa
sudo rm -Rf /poissonsearch
sudo mkdir -p /poissonsearch/qa/ && sudo chown jenkins /poissonsearch/qa/ && ln -s $PWD/qa/vagrant /poissonsearch/qa/
